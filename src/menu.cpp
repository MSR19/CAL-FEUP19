...
int Menu::intHandler(int max)
{
	...
	do
	{
		...
		if (std::cin.fail() || !(choice >= 1 && choice <= max))
		{
			...
			std::cout << std::endl
					  << "Input failed: try again!" << std::endl;
		}
...
void Menu::initialMenu()
{
 ...
 |___/\_,_/_//_/ /___/\___/_/ \__/|__,__/\_,_/_/  \__/ (_)                                                                                                                                                
		...
		std::cout << "6: Show colour index" << endl;
		...
		case 2:
			if (this->IsOnFirstIteration())
			{
				if (this->map != NULL)
					...
					std::cout << endl
							  << "you need to load a map first!" << endl;
			}
			break;
		case 3:
			if (this->IsOnFirstIteration())
		...
		case 7:
			std::cout << endl
					  << "Thank you for your preference!" << endl;
			this->map->exit();
			break;
		}
	} while (answer != 7);
...
void Menu::loadMap()
{
	...
	{
		std::cout << endl
				  << "You have already loaded a map, please restart the program to use a different map!" << endl;
	}
}
...
void Menu::changeNodesInVector(std::vector<Node *> nodes)
{
	...
	std::cout << endl
			  << "Chose the node that you want to alter (use the first number): ";
...
void Menu::chageNode(int nodeVectorPos)
{
	...
	if (this->IsOnFirstIteration())
	{
		...
		{
			cout << endl
				 << "Which of the following types do you want?" << endl;
			cout << "1: NONE" << endl;
			cout << "2: BANCOS" << endl;
			cout << "3: MUSEUS" << endl;
...
void Menu::showSolution()
{
	...
	this->showNodes(nodes);
	cout << endl
		 << "Chose the node that you want to start (use the first number): ";
	int node = this->intHandler(nodes.size());
	...
	cout << endl
		 << "Write anything to go back to main menu: ";
...
void Menu::multipleCars()
{
	...
	do
	{
		...
		std::cout << "5: Show colour index" << endl;
...
void Menu::carMails()
{
	...
	do
	{
		std::cout << "What do you wish to do?" << endl
				  << endl;
		std::cout << "1: Add post office" << endl;
		std::cout << "2: View post offices" << endl;
		std::cout << "3: View the shortest way" << endl;
		std::cout << "4: Back" << endl
				  << endl;
...
void Menu::colourIndex(int iteration)
{
	if (iteration == 0)
	{
		std::cout << "############### Colour Index ###############" << endl;
		std::cout << "Points of intererest: Black nodes" << endl;
		std::cout << "Path travelled: White nodes" << endl;
		std::cout << "Not travelled: Blue nodes" << endl
				  << endl;
	}
	else
	{
		std::cout << "############### Colour Index ###############" << endl;
		std::cout << "Banks: Green nodes" << endl;
		std::cout << "Path travelled to Banks: Cyan nodes" << endl;
		std::cout << "Museums: Orange nodes" << endl;
		std::cout << "Path travelled to Museums: Yellow nodes" << endl;
		std::cout << "Post Offices: Red nodes" << endl;
		std::cout << "Path travelled to Post Offices: Pink nodes" << endl;
		std::cout << "Councils: Dark Gray nodes" << endl;
		std::cout << "Path travelled to Councils: Light Gray nodes" << endl;
		std::cout << "Not travelled: Blue nodes" << endl
				  << endl;
	}
}
